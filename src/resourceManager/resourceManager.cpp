...
void ResourceManager::registerGame(Game *game) {
  printf("DEBUG: Entering resourceManager.cpp::*game)\n");
  ResourceManager::game = game;

  printf("DEBUG: Exiting resourceManager.cpp::*game)\n");
}

void ResourceManager::loadResources() {
  printf(
      "DEBUG: Entering resourceManager.cpp::ResourceManager::loadResources\n");
  // text
...
// Initialize SDL_mixer
  ...
  // music
  bgm = LinkedList<Music>({"./res/bgm/0.wav", "./res/bgm/1.wav",
                           "./res/bgm/2.wav", "./res/bgm/3.wav"});
  ...
  printf(
      "DEBUG: Exiting resourceManager.cpp::ResourceManager::loadResources\n");
}

void ResourceManager::freeResources() {
  printf(
      "DEBUG: Entering resourceManager.cpp::ResourceManager::freeResources\n");
  // images
  // icons
  delete WindowIcon;
  ...
  printf(
      "DEBUG: Exiting resourceManager.cpp::ResourceManager::freeResources\n");
}

void ResourceManager::playBGM(int index) {
  printf("DEBUG: Entering resourceManager.cpp::index)\n");
  ...
  if (index == STOP_MUSIC)
    ...
    printf("DEBUG: Entering resourceManager.cpp::START_MUSIC)\n");
    isPlaying = true;
#ifdef _WIN32
    PlaySound(bgm[playNum].data(), NULL, SND_FILENAME | SND_ASYNC | SND_LOOP);
#else
    Mix_Music *music = Mix_LoadMUS(bgm[playNum].data());
    if (music == NULL) {
    ...
    printf("DEBUG: Exiting resourceManager.cpp::START_MUSIC)\n");
  }
  ...
  if (!isPlaying) {
    printf("DEBUG: Entering resourceManager.cpp::\n");
...
// stop the background music
    ...
    printf("DEBUG: Exiting resourceManager.cpp::\n");
  } else if (playNum != wantPlay) {
    printf("DEBUG: Entering resourceManager.cpp::wantPlay)\n");
    // change the play music number
    playNum = wantPlay;
// play the background music
...
#else
    Mix_Music *music = Mix_LoadMUS(bgm[wantPlay].data());
    if (music == NULL) {
      printf("Failed to load music! SDL_mixer Error: %s\n", Mix_GetError());
      return;
    }
    ...
    printf("DEBUG: Exiting resourceManager.cpp::wantPlay)\n");
  }

  printf("DEBUG: Exiting resourceManager.cpp::index)\n");
}
