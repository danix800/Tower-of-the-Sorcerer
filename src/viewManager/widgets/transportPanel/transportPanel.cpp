...
TransportPanel::TransportPanel()
  ...
  printf("DEBUG: Entering transportPanel.cpp::TRANS_HEIGHT)\n");
  // set the group style
  this->box(FL_FLAT_BOX);
  this->color(ResourceManager::BackgroundColor);

  // title
  title = new Fl_Box(TRANS_X, TRANS_Y, TRANS_WIDTH, TRANS_TITLEH,
  ...
  // input
  input =
      new Fl_Int_Input(TRANS_INPUTX, TRANS_INPUTY, TRANS_INPUTW, TRANS_INPUTH);
  // set font
  input->textsize(TRANS_TEXTSIZE);
  input->textfont(ResourceManager::HeiTi);

  // button
  confirm = new Fl_Button(TRANS_BUTTONX, TRANS_BUTTONY, TRANS_BUTTONW,
                          TRANS_BUTTONH, "确定");
  // set style
  ...
  printf("DEBUG: Exiting transportPanel.cpp::TRANS_HEIGHT)\n");
}
...
void TransportPanel::transport(Fl_Widget *widget, void *data) {
  printf("DEBUG: Entering transportPanel.cpp::*data)\n");
  // get the game
  ...
  printf("DEBUG: Exiting transportPanel.cpp::*data)\n");
}

int TransportPanel::handle(int event) {
  printf("DEBUG: Entering transportPanel.cpp::event)\n");
  int ret;
  if (event == FL_KEYDOWN &&
  ...
  printf("DEBUG: Exiting transportPanel.cpp::event)\n");
}