...
TalkingUI::TalkingUI(LinkedList<TradeButton *> buttons)
    : Fl_Group(UI_X, UI_Y, UI_WIDTH, UI_HEIGHT), buttons(buttons) {
  ...
  // add name box
  nameBox = new Fl_Box(UI_X, UI_Y, UI_WIDTH, NAMEBOX_HEIGHT);
  // set style
  ...
  // add words box
  wordsBox = new TextDisplay(UI_X + UI_MARGIN, UI_Y + NAMEBOX_HEIGHT,
                             UI_WIDTH - 2 * UI_MARGIN, WORDSBOX_HEIGHT);
  // set color
  ...
  for (Node<TradeButton *> *p = buttons.begin(); p; p = p->getNext()) {
    ...
    button->resize(UI_X + margin,
                   UI_Y + NAMEBOX_HEIGHT + WORDSBOX_HEIGHT +
                       (cnt++) * (BUTTON_INTERVAL + BUTTON_HEIGHT),
...
void TalkingUI::updateText(string name, string words) {
  printf("DEBUG: Entering talkingUI.cpp::words)\n");
  nameBox->copy_label(name.data());
  wordsBox->setText(words.data());

  printf("DEBUG: Exiting talkingUI.cpp::words)\n");
}
