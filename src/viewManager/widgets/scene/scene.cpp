...
Scene::Scene() : Fl_Group(SCENE_X, SCENE_Y, SCENE_WIDTH, SCENE_HEIGHT) {
  printf("DEBUG: Entering scene.cpp::SCENE_HEIGHT)\n");
  // init map
  ...
  for (int i = 0; i < matrixWidth; i++)
    for (int j = 0; j < matrixWidth; j++) {
      Fl_Box *box =
          new Fl_Box(SCENE_X + i * cell_width, SCENE_Y + j * cell_height,
  ...
  printf("DEBUG: Exiting scene.cpp::SCENE_HEIGHT)\n");
}

Scene::~Scene() { this->clear(); }

void Scene::drawCell(Cell *cell) {
  printf("DEBUG: Entering scene.cpp::*cell)\n");
  // find the cell
  ...
  printf("DEBUG: Exiting scene.cpp::*cell)\n");
}

void Scene::drawHero(Hero *hero) {
  printf("DEBUG: Entering scene.cpp::*hero)\n");
  ...
  heroBox->position(SCENE_X + x * cell_width, SCENE_Y + y * cell_height);
  // redraw the hero
  heroBox->redraw();
  ...
  printf("DEBUG: Exiting scene.cpp::*hero)\n");
}