...
EnemyInfoBox::EnemyInfoBox(int x, int y, int w, int h, Enemy *enemy)
    : Fl_Group(x, y, w, h) {
  printf("DEBUG: Entering enemyInfoBox.cpp::h)\n");
  ...
  // photoBox
  photoBox = new Fl_Box(x + PHOTO_START_DX, y + PHOTO_START_DY, PHOTO_WIDTH,
                        PHOTO_HEIGHT);
  photoBox->box(FL_NO_BOX);
  photoBox->image(enemy->getPhoto());

  // nameBox
  nameBox = new Fl_Box(x + TEXT_START_DX, y, TEXTBOX_WIDTH * 3, TEXTBOX_HEIGHT);
  // set label text
  ...
  Fl_Box **boxList[] = {&healthBox, &attackBox, &defenceBox,
                        &damageBox, &moneyBox,  &expBox};
  // text to display in each box
  string labelList[] = {"生命：",       "攻击：", "防御：",
                        "对你的伤害：", "金钱：", "经验："};
  string valueList[] = {
      to_string(enemy->getHealth()),  to_string(enemy->getAttack()),
      to_string(enemy->getDefence()), to_string(enemy->getDamageTo(hero)),
      to_string(enemy->getMoney()),   to_string(enemy->getExp())};
  ...
  // add info boxes
  for (int i = 0; i < size; i++) {
    printf("DEBUG: Entering enemyInfoBox.cpp::i++)\n");
    int line = i / 3;
    int column = i % 3;
    Fl_Box *box = new Fl_Box(x + TEXT_START_DX + column * TEXTBOX_WIDTH,
                             y + (line + 1) * TEXTBOX_HEIGHT, TEXTBOX_WIDTH,
                             TEXTBOX_HEIGHT);
    // set label text
    ...
    *boxList[i] = box;
    ...
    printf("DEBUG: Exiting enemyInfoBox.cpp::i++)\n");
  }
  // finish adding widgets
  this->end();

  printf("DEBUG: Exiting enemyInfoBox.cpp::h)\n");
}
