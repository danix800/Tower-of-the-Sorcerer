...
BookPanel::BookPanel(LinkedList<Cell *> enemies)
    : Fl_Group(BOOK_X, BOOK_Y, BOOK_WIDTH, BOOK_HEIGHT), enemies(enemies),
      nowPage(0) {
  // set the group style
  ...
  // prevPage button
  prevPage = new Fl_Button(BOOK_BUTTONX, BOOK_BUTTONY, BOOK_BUTTONW,
                           BOOK_BUTTONH, "上一页");
  // set style
  ...
  // nextPage button
  nextPage = new Fl_Button(BOOK_BUTTONX + BOOK_BUTTONW + BOOK_BTN_INTERVAL,
                           BOOK_BUTTONY, BOOK_BUTTONW, BOOK_BUTTONH, "下一页");
  // set style
...
void BookPanel::updateBoxes() {
  printf("DEBUG: Entering bookPanel.cpp::BookPanel::updateBoxes\n");
  // free memory
  for (int i = 0; i < DISPLAY_NUM; i++)
    if (boxes[i] != nullptr) {
  ...
  for (int i = 0; p && i < DISPLAY_NUM; i++, p = p->getNext()) {
    EnemyInfoBox *box =
        new EnemyInfoBox(BOOK_X, BOOK_Y + i * ENEMYBOX_H, BOOK_WIDTH,
                         ENEMYBOX_H, (Enemy *)p->getValue());
    // add box to the panel
    this->add(box);
    boxes[i] = box;
  }

  printf("DEBUG: Exiting bookPanel.cpp::BookPanel::updateBoxes\n");
}

void BookPanel::goNextPage(Fl_Widget *widget, void *data) {
  printf("DEBUG: Entering bookPanel.cpp::*data)\n");
  // get the panel
  ...
  printf("DEBUG: Exiting bookPanel.cpp::*data)\n");
}
...
void BookPanel::goPrevPage(Fl_Widget *widget, void *data) {
  ...
  if (panel->nowPage > 0) {
    printf("DEBUG: Entering bookPanel.cpp::0)\n");
    panel->nowPage--;
    panel->updateBoxes();
    panel->redraw();
    ResourceManager::game->printStatus("上一页");

    printf("DEBUG: Exiting bookPanel.cpp::0)\n");
  }
