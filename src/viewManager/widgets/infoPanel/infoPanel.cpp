...
InfoPanel::InfoPanel()
  ...
  printf("DEBUG: Entering infoPanel.cpp::INFOPANEL_HEIGHT)\n");
  // init values
  ...
  // titleBox
  titleBox = new Fl_Box(0, INFOPANEL_Y, INFOPANEL_WIDTH, LINE_HEIGHT);
  // style
  ...
  const char *labels[] = {"等级", "生命", "攻击", "防御", "金币", "经验"};
  // get number of labels
  int num = sizeof(labels) / sizeof(labels[0]);
  for (int i = 0; i < num; i++) {
    Fl_Box *box = new Fl_Box(MARGIN, INFOPANEL_Y + (i + 2) * LINE_HEIGHT,
                             INFOPANEL_WIDTH / 3 - MARGIN, LINE_HEIGHT);
    // style
    box->box(FL_NO_BOX);
  ...
  Fl_Box **boxes[] = {&levelBox,   &healthBox, &attackBox,
                      &defenceBox, &moneyBox,  &expBox};
  for (int i = 0; i < num; i++) {
    Fl_Box *box =
        new Fl_Box(INFOPANEL_WIDTH / 3, INFOPANEL_Y + (i + 2) * LINE_HEIGHT,
                   INFOPANEL_WIDTH / 3 * 2 - MARGIN, LINE_HEIGHT);
    // style
    box->box(FL_NO_BOX);
  ...
  // key images boxes
  Fl_Image *keyImages[] = {ResourceManager::RedKeyTrans,
                           ResourceManager::BlueKeyTrans,
                           ResourceManager::YellowKeyTrans};
  int keyNum = sizeof(keyImages) / sizeof(keyImages[0]);
  // add key images
  for (int i = 0; i < keyNum; i++) {
    printf("DEBUG: Entering infoPanel.cpp::i++)\n");
    Fl_Box *box = new Fl_Box(INFOPANEL_WIDTH / keyNum * i,
                             INFOPANEL_Y + (num + 3) * LINE_HEIGHT,
                             INFOPANEL_WIDTH / keyNum, LINE_HEIGHT);
    // style
    box->box(FL_NO_BOX);
    // image
    ...
    printf("DEBUG: Exiting infoPanel.cpp::i++)\n");
  }

  // to reduce the code,
  // use an array to modify each keybox
  Fl_Box **keyBoxes[] = {&redKeysBox, &blueKeysBox, &yellowKeysBox};
  for (int i = 0; i < keyNum; i++) {
    Fl_Box *box = new Fl_Box(INFOPANEL_WIDTH / keyNum * i,
  ...
  printf("DEBUG: Exiting infoPanel.cpp::INFOPANEL_HEIGHT)\n");
}

InfoPanel::~InfoPanel() { this->clear(); }

void InfoPanel::updateTowerLevel(int towerLevel) {
  printf("DEBUG: Entering infoPanel.cpp::towerLevel)\n");
  this->tower_level = towerLevel;

  printf("DEBUG: Exiting infoPanel.cpp::towerLevel)\n");
}

void InfoPanel::updateHeroInfo(Hero *hero) {
  printf("DEBUG: Entering infoPanel.cpp::*hero)\n");
  ...
  printf("DEBUG: Exiting infoPanel.cpp::*hero)\n");
}

void InfoPanel::updateKeys(int red, int blue, int yellow) {
  printf("DEBUG: Entering infoPanel.cpp::yellow)\n");
  redKeys = red;
  blueKeys = blue;
  yellowKeys = yellow;

  printf("DEBUG: Exiting infoPanel.cpp::yellow)\n");
}

void InfoPanel::updateValueTexts() {
  printf("DEBUG: Entering infoPanel.cpp::InfoPanel::updateValueTexts\n");
  // set title
  ...
  printf("DEBUG: Exiting infoPanel.cpp::InfoPanel::updateValueTexts\n");
}